...
fn contains_check(a: &dyn ValueView, b: &dyn ValueView) -> Result<bool> {
    if let Some(a) = a.as_scalar() {
        ...
        // Ruby Liquid: `nil contains "anything"` returns false.
        // This matches Jekyll's lenient behavior where undefined variables
        // in contains checks silently evaluate to false.
        Ok(false)
    } else {
