...
//!
//! Run with: cargo test -p integration-tests --test integration_feed_sitemap
...
fn dtc_site_dir() -> PathBuf {
    let p = project_root().join("websites/DataTalksClub/datatalksclub.github.io");
    if p.exists() {
        return p;
    }
    // Fallback to old location
    project_root().join("datatalksclub.github.io")
}

fn kids_site_dir() -> PathBuf {
    project_root().join("websites/alexeygrigorev/kids-horror-stories-ru")
}
...
/// picked up correctly.
/// Returns `None` if the source directory is missing or Jekyll is not available.
fn build_with_jekyll(source: &Path, dest: &Path) -> Option<()> {
    if !source.exists() {
        eprintln!("SKIPPING: source not found at {:?}", source);
        return None;
    }
    ...
    let output = match Command::new("bundle")
        .arg("exec")
        .arg("jekyll")
        .arg("build")
        .arg("--destination")
        ...
        .output()
    {
        Ok(o) => o,
        Err(e) => {
            eprintln!("SKIPPING: failed to run bundle exec jekyll: {}", e);
            return None;
        }
    };
    ...
    if !output.status.success() {
        eprintln!(
            "SKIPPING: Jekyll build failed for {}.\nstdout: {}\nstderr: {}",
            source.display(),
            stdout,
            stderr
        );
...
fn test_dtc_sitemap_vs_jekyll() {
    ...
    if !jekyll_sitemap_path.exists() {
        eprintln!("SKIPPING: Jekyll did not produce sitemap.xml");
        return;
    }
