...
impl Renderable for SeoRenderable {
    fn render_to(&self, writer: &mut dyn Write, runtime: &dyn Runtime) -> liquid_core::Result<()> {
        ...
        // Jekyll's jekyll-seo-tag does NOT fall back to page content -- if none of
        // these are set, no description meta tag is emitted (issue 530).
...
mod tests {
    ...
    fn test_seo_no_description_from_page_content() {
        // Issue 530: Jekyll does NOT fall back to page.content for description.
        // When only content exists (no description, no excerpt, no site.description),
        // no description meta tag should be emitted.
        let eng = engine();
        let mut ctx = Object::new();
        let mut page = Object::new();
        let site = Object::new();
        ...
        assert!(
            ...
            "No description meta tag when only page.content exists. Got: {}",
            out
        );
    }

    #[test]
    fn test_seo_no_description_from_content_with_html() {
        // Issue 530: Even with HTML content, no description should be generated
        // from page content alone.
        let eng = engine();
        let mut ctx = Object::new();
        let mut page = Object::new();
        let site = Object::new();
        ...
        assert!(
            !out.contains("name=\"description\""),
            "No description from content-only page. Got: {}",
            out
        );
    }
    ...
    fn test_seo_no_description_from_content_unicode() {
        // Issue 530: Unicode content should also not produce a description fallback.
        let eng = engine();
        let mut ctx = Object::new();
        let mut page = Object::new();
        let site = Object::new();
        ...
        assert!(
            !out.contains("name=\"description\""),
            "No description from unicode content. Got: {}",
            out
        );
    }
    ...
    fn test_seo_no_description_from_long_content() {
        // Issue 530: Long content should not produce a truncated description snippet.
        let eng = engine();
        let mut ctx = Object::new();
        let mut page = Object::new();
        let site = Object::new();
        ...
        assert!(
            !out.contains("name=\"description\""),
            "No description from long content. Got: {}",
            out
        );
    }
    ...
    fn test_issue530_no_description_from_content_for_non_post_page() {
        // A page with no description, no excerpt, no site.description should NOT
        // get a description meta tag from page content (Jekyll behavior).
        let eng = engine();
        let mut ctx = Object::new();
        let mut page = Object::new();
        let site = Object::new();
        page.insert("title".into(), Value::scalar("404 Not Found".to_string()));
        page.insert(
            "content".into(),
            Value::scalar("<p>Sorry, the page you are looking for does not exist.</p>".to_string()),
        );
        ctx.insert("page".into(), Value::Object(page));
        ctx.insert("site".into(), Value::Object(site));
        let out = eng.parse_and_render("{% seo %}", &ctx).unwrap();
        assert!(
            !out.contains("name=\"description\""),
            "Non-post page without explicit description should NOT have meta description. Got: {}",
            out
        );
        assert!(
            !out.contains("og:description"),
            "Non-post page without explicit description should NOT have og:description. Got: {}",
            out
        );
    }

    #[test]
    fn test_issue530_no_description_in_jsonld_when_no_explicit_description() {
        // JSON-LD should also omit "description" when there's no explicit description
        let eng = engine();
        let mut ctx = Object::new();
        let mut page = Object::new();
        let site = Object::new();
        page.insert("title".into(), Value::scalar("Index".to_string()));
        page.insert(
            "content".into(),
            Value::scalar("<p>Welcome to my site</p>".to_string()),
        );
        ctx.insert("page".into(), Value::Object(page));
        ctx.insert("site".into(), Value::Object(site));
        let out = eng.parse_and_render("{% seo %}", &ctx).unwrap();
        ...
        assert!(
            ...
            "JSON-LD should NOT contain description when no explicit description set. Got: {}",
            jsonld
        );
    }

    #[test]
    fn test_issue530_explicit_description_still_emitted() {
        // Pages with explicit front matter description should still work
        let eng = engine();
        let mut ctx = Object::new();
        let mut page = Object::new();
        let site = Object::new();
        page.insert(
            "description".into(),
            Value::scalar("My explicit description".to_string()),
        );
        page.insert(
            "content".into(),
            Value::scalar("<p>Some content</p>".to_string()),
        );
        ctx.insert("page".into(), Value::Object(page));
        ctx.insert("site".into(), Value::Object(site));
        let out = eng.parse_and_render("{% seo %}", &ctx).unwrap();
        assert!(
            out.contains("name=\"description\""),
            "Page with explicit description should have meta description. Got: {}",
            out
        );
        assert!(
            out.contains("My explicit description"),
            "Description should contain the explicit text. Got: {}",
            out
        );
    }

    #[test]
    fn test_issue530_site_description_still_used_as_fallback() {
        // site.description should still be used when no page description/excerpt
        let eng = engine();
        let mut ctx = Object::new();
        let mut page = Object::new();
        let mut site = Object::new();
        ...
        page.insert(
            "content".into(),
            Value::scalar("<p>About page content</p>".to_string()),
        );
        site.insert(
            "description".into(),
        ...
        assert!(
            ...
            "site.description should be used as fallback. Got: {}",
            out
        );
    }
}
