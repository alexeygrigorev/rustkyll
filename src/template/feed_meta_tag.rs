...
//!
//! Generates the main `<link>` tag for the Atom/RSS feed based on site configuration:
//! - Main feed link using `site.url` + feed path (default `feed.xml`)
//! - Title from `site.name` or `site.title`
//!
//! Note: Jekyll's real feed_meta only outputs the main feed link.
//! Category-specific feed links are NOT generated by feed_meta;
//! sites add those manually in their templates.
...
impl Renderable for FeedMetaRenderable {
    fn render_to(&self, writer: &mut dyn Write, runtime: &dyn Runtime) -> liquid_core::Result<()> {
        ...
        // Category-specific feed links are NOT generated by feed_meta;
        // sites that want them add manual <link> tags in their templates.
...
mod tests {
    ...
    #[test]
    fn test_feed_meta_with_categories_only_main_link() {
        // Jekyll's feed_meta only outputs the main feed link, NOT category-specific links.
        // Category feed links are added manually in site templates.
        let eng = engine();
        let mut ctx = Object::new();
        let mut feed_obj = liquid::Object::new();
        feed_obj.insert(
        ...
        assert!(
            !out.contains("feed/release.xml"),
            "Should NOT have category feed link (Jekyll only outputs main feed): {}",
            out
        );
    }
